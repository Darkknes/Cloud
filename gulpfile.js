var gulp         = require('gulp'),
	...
	uncss		 = require('gulp-uncss-sp'),//Удаление лишниго css
	htmlmin		 = require('gulp-htmlmin'),//Минификация html
	csscomb		 = require('gulp-csscomb'),//Исходное css
	imacss 		 = require('gulp-imacss'),//DATE64
	notify 		 = require("gulp-notify"),//Ошибки
...
gulp.task('imacss', function () {//DATE64
    gulp.src('app/img/*.jpg')
        .pipe(imacss('app/css/style.css'))
        .pipe(gulp.dest('app/css/date'));
});


gulp.task('styles', function() {//Исходное css
  return gulp.src('app/css/style.css')
    .pipe(csscomb())
    .pipe(gulp.dest('app/css/'));
});

gulp.task('minhtml', () => {//Минификация html
...
gulp.task('st', ['browser-sync', 'css-libs', 'scripts'], function() {
...
gulp.task('build', ['clean','img', 'scripts','css-libs'], function() {
	...
	var buildJs = gulp.src('app/**/**/*') // Переносим скрипты в продакшен
	.pipe(gulp.dest('dist'))
